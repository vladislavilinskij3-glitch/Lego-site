...
Этот сайт — фронтенд-версия. Чтобы сделать его полноценным бизнес-инструментом, нужно добавить серверную часть (Backend):

1. **Настоящая регистрация и вход.** Сейчас данные пользователей хранятся только в браузере (LocalStorage). Если человек почистит кэш, аккаунт исчезнет. Хранить пользователей нужно  на сервере, а при регистрации отправлять на почту код подтверждения.
